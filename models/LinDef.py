...
class LinDef(nn.Module):
    ...
    def forward(self, x, inp=6, k=8, flag_step2=False, x2=0):
        down1 = []
        pool_temp = []
        for j in range(self.n_blocks + 1):
            ...
            for i in range(k):
                if j > 0:
                    ...
                    print('joint pool:'+str(joint_pool.shape))
                    print('pool_temp[0]:'+str(pool_temp[0].shape))
                    print('pool_max[0]:'+str(pool_max[0].shape))
                    pool_temp.pop(0)
                else:
